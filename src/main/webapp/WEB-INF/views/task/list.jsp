...
<table class="table table-striped">
    ...
    <c:forEach items="${tasks}" var="task">
        <tr>
                            ...
                            <c:if test="${user.id == taskOperation.user.id}">
                                <td>
                                    <a class="btn btn-outline-warning float-right"
                                       href="http://localhost:8080/taskoperations/edit/${taskOperation.id}">Edytuj</a>
                                    <a class="btn btn-outline-danger float-right"
                                       href="http://localhost:8080/taskoperations/delete/${taskOperation.id}">Usuń</a>
                                </td>
                            </c:if>
                        </tr>
                    </c:forEach>
            ...
            <td>
                <a class="btn btn-outline-success" href="http://localhost:8080/taskoperations/add/${task.id}">Dodaj
                    działanie</a>
                <a class="btn btn-outline-warning" href="http://localhost:8080/task/edit/${task.id}">Edytuj zadanie</a>
                <c:if test="${empty task.getTaskOperations()}">
                    <a class="btn btn-outline-danger" href="http://localhost:8080/task/delete/${task.id}">Usuń
                        zadanie</a>
                </c:if>
            </td>
        </tr>
